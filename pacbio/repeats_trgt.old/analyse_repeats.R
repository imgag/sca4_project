...
files <- list.files("pacbio/repeats_trgt", pattern="repeat_catalog.GRCh38.vcf.gz")

f <- "pacbio/repeats_trgt/22018LRa001.repeat_catalog.GRCh38.vcf.gz"
f <- "/mnt/storage1/projects/research/22042I_1011_Repeat_Neuro/Sample_22042LRa003/repeats_trgt/22042LRa003.vcf.gz" 
out_prefix <- "pacbio/repeats_trgt/22042"
plot_overview <- TRUE
create_folders <- TRUE
...
if (create_folders) {
    for (i in seq_along(tvf$TRID)) {
        row <- tvf[i,]
        outfile <- paste0(out_prefix, "_plot/", row$TRID,"/repeat_locus_info.tsv")
